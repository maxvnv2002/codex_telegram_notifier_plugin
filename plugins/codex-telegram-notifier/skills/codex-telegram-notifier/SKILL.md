...
- If the user did not provide a code, ask them to get one from the Telegram bot with `/newcode`.
- Locate the installed plugin script under Codex's plugin cache.
- Run setup with the pairing code.
...
The setup command registers the device with the backend and stores device state in `~/.codex-telegram-notifier/config.json`.
